...
func main() {
	...
	upLeft := image.Point{0, 0}            // the image import
	lowRight := image.Point{width, height} // the image import
	...
	for x := 0; x < width; x++ {
		for y := 0; y < height; y++ {
			...
			// pixelColor := color.RGBA{uint8(float64(x) / float64(width) * 255), uint8(float64(y) / float64(height) * 255), 80, 255} // the "image/color" import
			pixelColor := color.RGBA{150, 0, 250, 255}
	...
	defer f.Close()
	png.Encode(f, img) // the "image/png" import
}
